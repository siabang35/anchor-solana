...
export class RSSClient {
    ...
    /**
     * Fetch Economy/Business news from Google News
     */
    async fetchGoogleNewsEconomy(): Promise<MarketDataItem[]> {
        const url = 'https://news.google.com/rss/headlines/section/topic/BUSINESS?hl=en-US&gl=US&ceid=US:en';
        return this.fetchFeed(url, 'Google News Business', 'economy');
    }

    /**
     * Fetch Economy news from CNBC
     */
    async fetchCNBCEconomy(): Promise<MarketDataItem[]> {
        const url = 'https://search.cnbc.com/rs/search/combinedcms/view.xml?partnerId=wrss01&id=20910258';
        return this.fetchFeed(url, 'CNBC Economy', 'economy');
    }

    /**
     * Fetch Business/Economy news from Wall Street Journal
     */
    async fetchWSJEconomy(): Promise<MarketDataItem[]> {
        const url = 'https://feeds.a.dj.com/rss/WSJcomUSBusiness.xml';
        return this.fetchFeed(url, 'Wall Street Journal', 'economy');
    }

    /**
     * Fetch Global Economy news from Financial Times
     */
    async fetchFTEconomy(): Promise<MarketDataItem[]> {
        const url = 'https://www.ft.com/global-economy?format=rss';
        return this.fetchFeed(url, 'Financial Times', 'economy');
    }

    /**
     * Fetch Economy news from Investing.com
     */
    async fetchInvestingEconomy(): Promise<MarketDataItem[]> {
        const url = 'https://www.investing.com/rss/news_285.rss';
        return this.fetchFeed(url, 'Investing.com', 'economy');
    }

    /**
     * Fetch all economy feeds concurrently
     */
    async fetchAllEconomyFeeds(): Promise<MarketDataItem[]> {
        const results = await Promise.allSettled([
            this.fetchGoogleNewsEconomy(),
            this.fetchCNBCEconomy(),
            this.fetchWSJEconomy(),
            this.fetchFTEconomy(),
            this.fetchInvestingEconomy()
        ]);
        ...
        for (const result of results) {
            if (result.status === 'fulfilled') {
                ...
                this.logger.warn(`Economy feed fetch failed: ${result.reason}`);
            }
        }
